...
static SpriteTransform crate_sprites[NUM_CRATES] = {
    { .pos = { -100.0f, 50.0f }, .rotation = 15.0f, .scale = { 80.0f, 80.0f } },
    { .pos = { 0.0f, -20.0f }, .rotation = -30.0f, .scale = { 100.0f, 100.0f } },
    { .pos = { 110.0f, 60.0f }, .rotation = 45.0f, .scale = { 70.0f, 70.0f } }
};
...
/*
 * Normal texture orientation.
 *
 * Used for the crate texture.
 */
...
/*
 * Text texture orientation.
 *
 * SDL_ttf surface requires the V coordinate to be flipped.
 *
 * U stays unchanged, so the text is NOT mirrored horizontally.
 * V is reversed, so the text is NOT upside down.
 */
...
static bool InitPipeline(void)
{
    ...
    WGPUBindGroupEntry crate_bg_entries[3] = { { 0 } };
    ...
    WGPUBindGroupDescriptor crate_bg_desc = { 0 };
    ...
    WGPUBindGroupEntry text_bg_entries[3] = { { 0 } };
    ...
    WGPUBindGroupDescriptor text_bg_desc = { 0 };
     ...
     * Text pipeline.
     *
     * This pipeline uses V-flipped UVs.
     */
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow(
        "integrate-sdl3-ttf-and-sdl-image-webgpu-c",
        WIN_WIDTH,
        WIN_HEIGHT,
...
#define SURFACE_STATUS_SUCCESS(s) ((s) == 0 || (s) == 1)
    ...
    colorAttachment.clearValue = (WGPUColor) { 0.15, 0.15, 0.18, 1.0 };
    ...
    for (int i = 0; i < NUM_CRATES; i++)
    {
        ...
        glm_rotate(model, glm_rad(crate_sprites[i].rotation), (vec3) { 0.0f, 0.0f, 1.0f });
